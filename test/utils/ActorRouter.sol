...
/// @notice A dedicated, single-purpose swap executor. Deployed once per
/// simulated actor (searcher, victim, ...) in tests, because PoolManager
/// attributes swap identity to whichever contract directly calls
/// `swap()` (V4_ARCHITECTURE_VALIDATION.md §1) — routing every actor through
/// one shared test router would collapse all of their identities into a
/// single address and make sandwich detection untestable.
contract ActorRouter is IUnlockCallback {
